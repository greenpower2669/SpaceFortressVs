...
// 2026 remaster layer. Loaded after start_ui.hpp so the historical main.cpp
// remains untouched. Visuals and Android compatibility stay at the boundary.
                             ...
                             Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
    ...
    for (int y = -radius; y <= radius; ++y) {
        const int rr = radius * radius - y * y;
        const int x = static_cast<int>(std::sqrt(static_cast<double>(std::max(0, rr))));
...
static void sfRmDrawShootingStar(SDL_Renderer *renderer, int width, int height)
{
    ...
    const float t = static_cast<float>(age) / static_cast<float>(sfRmStar.duration);
    const float x = sfRmStar.x + sfRmStar.vx * t;
    const float y = sfRmStar.y + sfRmStar.vy * t;
    const float speed = std::sqrt(sfRmStar.vx * sfRmStar.vx + sfRmStar.vy * sfRmStar.vy);
    ...
    SDL_SetRenderDrawColor(renderer, 130, 195, 255, static_cast<Uint8>(110 * fade));
    SDL_RenderDrawLine(renderer,
                       static_cast<int>(x - ux * len),
                       static_cast<int>(y - uy * len),
                       static_cast<int>(x), static_cast<int>(y));
    SDL_SetRenderDrawColor(renderer, 245, 252, 255, static_cast<Uint8>(245 * fade));
    SDL_RenderDrawLine(renderer,
...
static void sfRmDrawScenicBodies(SDL_Renderer *renderer, int width, int height,
                                 Uint64 now)
{
    // Sun and planet are no longer painted into a background bitmap. They are
    // independent, slow scene events with long empty-space intervals.
    ...
    if (cycle >= 24500u && cycle <= 40500u) {
        ...
        const int cx = static_cast<int>(width + radius * 0.70f - reveal * radius * 0.84f);
...
static void sfRmDrawLiveBackground(SDL_Renderer *renderer)
{
    ...
    // Three moving parallax star layers with independent twinkle.
    const int counts[3] = {58, 42, 24};
    const float speeds[3] = {0.0028f, 0.0060f, 0.0115f};
    for (int layer = 0; layer < 3; ++layer) {
        for (int i = 0; i < counts[layer]; ++i) {
            ...
            float ny = std::fmod(baseY + static_cast<float>(seconds) * speeds[layer], 1.0f);
            if (ny < 0.0f) ny += 1.0f;
            const float sway = 0.006f * layer *
                std::sin(static_cast<float>(seconds * 0.10 + i));
            const float nx = std::fmod(baseX + sway + 1.0f, 1.0f);
            ...
            if (layer == 0) SDL_SetRenderDrawColor(renderer, 150, 180, 210, alpha);
            else if (layer == 1) SDL_SetRenderDrawColor(renderer, 190, 220, 245, alpha);
            else SDL_SetRenderDrawColor(renderer, 235, 247, 255, alpha);
...
// Defined before the macro below, therefore SDL_RenderClear here is the real SDL call.
...
{
    sfRmGear1Down = sfRmGear2Down = false;
    sfRmGear1Finger = sfRmGear2Finger = 0;
}
...
static void sfRmResetAiAnchor()
{
    ...
    // Historical IA copies iago->xy back into Spritej1. Reset that invisible
    // anchor on launch so background simulation cannot leave the IA off-screen.
...
static void sfRmSyncUiEngineState()
{
    if (sfUiScreen != SF_UI_GAME) {
        // setgui is the old engine's own GUI/control gate. Holding it here keeps
...
static int SpaceFortressRemaster_WaitEvent(SDL_Event *event)
{
    ...
    if (event->type == SDL_FINGERDOWN) {
        ...
        if (hit1) { sfRmGear1Down = true; sfRmGear1Finger = fid; event->type = SDL_USEREVENT; }
        if (hit2) { sfRmGear2Down = true; sfRmGear2Finger = fid; event->type = SDL_USEREVENT; }
        if (sfRmGear1Down && sfRmGear2Down && sfRmGear1Finger != sfRmGear2Finger) {
            sfRmGoHome();
                              ...
                              Uint8 r, Uint8 g, Uint8 b, bool inverted)
{
    ...
    const float wave = 0.5f + 0.5f * std::sin(SDL_GetTicks64() * 0.006);
    const int size = static_cast<int>(std::max(ship->w, ship->h) * (0.16f + 0.035f * wave));
    if (size <= 0) return;
    const int frameW = std::max(1, sfRmMuzzleW / 4);
    ...
    SDL_SetTextureAlphaMod(sfRmMuzzleStrip, static_cast<Uint8>(75 + wave * 70));
